...
class PDFBarcode
{
	...
	protected function checksum_code93($code)
	{
		...
		$code = strtr($code, barcode . phpchr(128) . chr(131) . chr(129) . chr(130), '<=>?');
		$len = strlen($code);
		...
		$checksum = strtr($checksum, '<=>?', barcode . phpchr(128) . chr(131) . chr(129) . chr(130));
		return $checksum;
	}
	...
	protected function barcode_c128($code, $type = 'B', $ean = false)
	{
		...
		$code = barcode . phpchr($startid) . $code . chr($check) . chr(106) . chr(107);
		$bararray = array('code' => $code, 'maxw' => 0, 'maxh' => 1, 'bcode' => array());
		$k = 0;
		$len = strlen($code);
		for ($i = 0; $i < $len; ++$i) {
	...
	protected function barcode_eanupc($code, $len = 13)
	{
		...
		if ($upce) {
			...
			if (($tmp == '000') OR ( $tmp == '100') OR ( $tmp == '200')) {
				...
				$upce_code = barcode . phpsubstr($code, 2, 2) . substr($code, 9, 3) . substr($code, 4, 1);
				if ($prod_code > 999) {
					$invalid_upce = true;
				}
			} else {
				...
				if ($tmp == '00') {
					...
					$upce_code = barcode . phpsubstr($code, 2, 3) . substr($code, 10, 2) . '3';
					if ($prod_code > 99) {
						$invalid_upce = true;
					}
				} else {
					...
					if ($tmp == '0') {
						...
						$upce_code = barcode . phpsubstr($code, 2, 4) . substr($code, 11, 1) . '4';
						if ($prod_code > 9) {
							$invalid_upce = true;
						}
					} else {
						// manufacturer code does not end in zero
						$upce_code = barcode . phpsubstr($code, 2, 5) . substr($code, 11, 1);
						if ($prod_code > 9) {
							$invalid_upce = true;
						}
					}
