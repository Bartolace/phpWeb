...
class TTFontFile
{
		...
		foreach ($this->tables AS $t) {
			if ($t['length'] > 0 && $t['length'] < $this->maxStrLenRead) { // 1.02
				...
				if ($xchecksum != $checksum) {
					throw new MpdfException(sprintf('TTF file "%s": invalid checksum %s table: %s (expected %s)', $this->filename, ttfontsuni . phpdechex($checksum[0]) . dechex($checksum[1]), $t['tag'], dechex($xchecksum[0]) . dechex($xchecksum[1])));
				}
			}
		}
	}
	...
	{
		return ttfontsuni . phpsubstr($stream, 0, $offset) . $value . substr($stream, $offset + strlen($value));
	}
...
?>';
	...
	{ // mPDF 5.7.1
		...
		if (isset($this->tables['OS/2'])) {
			...
			$os2 = $this->splice($os2, 32, ttfontsuni . phpchr(5) . chr(0) . chr(1) . chr(0) . chr(1) . chr(0) . chr(0) . chr(0) . chr(0) . chr(0));
			$this->add('OS/2', $os2);
		}
