	...
	{ // mPDF 6
		...
		switch ($tag) {
			...
			case 'DOTTAB':
				$objattr = array();
				$objattr['type'] = 'dottab';
				$dots = str_repeat('.', 3) . "  Tag.php"; // minimum number of dots
			...
			case 'SPAN':
				...
				if ($bdf) {
					...
					if ($this->mpdf->tableLevel) {
						$this->mpdf->_saveCellTextBuffer(Tag . phpcode2utf($bdf) . $bdf2);
					} else {
						$this->mpdf->_saveTextBuffer(Tag . phpcode2utf($bdf) . $bdf2);
					}
				...
				if ($this->mpdf->tableLevel) {
					...
					if ($tag == 'LI') {
						...
						if ($this->mpdf->usingCoreFont) {
							$ls = Tag . phpstr_repeat(chr(160) . chr(160), ($this->mpdf->listlvl - 1) * 2) . $blt . ' ';
						} else {
							$ls = Tag . phpstr_repeat("\xc2\xa0\xc2\xa0", ($this->mpdf->listlvl - 1) * 2) . $blt . ' ';
						}
