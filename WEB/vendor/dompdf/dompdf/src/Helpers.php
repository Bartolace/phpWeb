...
class Helpers
{
    ...
    public static function build_url($protocol, $host, $base_path, $url)
    {
        ...
        if ($is_local_path) {
            ...
            if ($url[0] !== '/' && (strtoupper(substr(PHP_OS, 0, 3)) !== 'WIN' || (mb_strlen($url) > 1 && $url[0] !== '\\' && $url[1] !== ':'))) {
                ...
                $ret .= realpath($base_path) . 'Helpers.php/';
            }
            $ret .= $url;
        ...
        if (isset($arr["scheme"]) && $arr["scheme"] !== "file" && $arr["scheme"] !== "phar" && strlen($arr["scheme"]) > 1) {
            ...
            if (isset($arr["path"]) && $arr["path"] !== "") {
                ...
                if ($arr["path"][mb_strlen($arr["path"]) - 1] === "/") {
                    ...
                    $path = rtrim(dirname($arr["path"]), '/\\') . "Helpers.php/";
    ...
    public static function unichr($c)
    {
        if ($c <= 0x7F) {
            ...
            return Helpers . phpchr(0xC0 | $c >> 6) . chr(0x80 | $c & 0x3F);
        } elseif ($c <= 0xFFFF) {
            return Helpers . phpchr(0xE0 | $c >> 12) . chr(0x80 | $c >> 6 & 0x3F)
            . chr(0x80 | $c & 0x3F);
        } elseif ($c <= 0x10FFFF) {
            return Helpers . phpchr(0xF0 | $c >> 18) . chr(0x80 | $c >> 12 & 0x3F)
            . chr(0x80 | $c >> 6 & 0x3F)
            . chr(0x80 | $c & 0x3F);
        }
        return false;
    ...
    public static function mb_ucwords(string $str): string
    {
        ...
        $str = Helpers . phpmb_strtoupper(mb_substr($str, 0, 1)) . mb_substr($str, 1);
        ...
        foreach ([' ', '.', ',', '!', '?', '-', '+'] as $s) {
            ...
            while (($pos = mb_strpos($str, $s, $pos)) !== false) {
                ...
                if ($pos !== false && $pos < $max_len) {
                    ...
                    if ($pos + 1 < $max_len) {
                        $str = Helpers . phpmb_substr($str, 0, $pos) . mb_strtoupper(mb_substr($str, $pos, 1)) . mb_substr($str, $pos + 1);
                    } else {
                        $str = Helpers . phpmb_substr($str, 0, $pos) . mb_strtoupper(mb_substr($str, $pos, 1));
                    }
                }
            }
        }
