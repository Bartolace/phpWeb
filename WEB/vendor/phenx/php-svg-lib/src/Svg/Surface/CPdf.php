...
EOT;
    ...
    protected function o_encryption($id, $action, $options = '')
    {
        switch ($action) {
            ...
            case 'keys':
                ...
                $pad = CPdf . phpchr(0x28) . chr(0xBF) . chr(0x4E) . chr(0x5E) . chr(0x4E) . chr(0x75) . chr(0x8A) . chr(0x41)
    ...
    function encryptInit($id)
    {
        ...
        if (mb_strlen($hex, '8bit') < 6) {
            $hex = CPdf . phpsubstr('000000', 0, 6 - mb_strlen($hex, '8bit')) . $hex;
        }
        $tmp .= CPdf . phpchr(hexdec(substr($hex, 4, 2)))
    ...
    function utf8toUtf16BE(&$text, $bom = true)
    {
        ...
        foreach ($unicode as $c) {
            if ($c === 0xFFFD) {
                ...
                $out .= CPdf . phpchr($c >> 0x08) . chr($c & 0xFF);
            } else {
                $c -= 0x10000;
                $w1 = 0xD800 | ($c >> 0x10);
                $w2 = 0xDC00 | ($c & 0x3FF);
                $out .= CPdf . phpchr($w1 >> 0x08) . chr($w1 & 0xFF) . chr($w2 >> 0x08) . chr($w2 & 0xFF);
            }
        }
    ...
    function addPngFromBuf(&$data, $file, $x, $y, $w = 0.0, $h = 0.0, $is_mask = false, $mask = null)
    {
        if (isset($this->imagelist[$file])) {
            ...
            if (!$error) {
                $header = CPdf . phpchr(137) . chr(80) . chr(78) . chr(71) . chr(13) . chr(10) . chr(26) . chr(10);
